...
from ..helpers.translate import translate_post  # ZCode: 翻译
from ..helpers.push_history import record as record_push, init as init_push_history  # ZCode: 推送历史
from ..i18n import i18n
from ..parsing.post import get_post_from_entry, Post
from ..parsing.post_formatter import PostFormatter  # ZCode: 用于重建翻译后的 formatter
...
class Notifier:
    ...
    @classmethod
    def init_push_history_db(cls, config_dir: str):
        """Initialize push history DB (called from entrypoint)"""
        init_push_history(config_dir)
    ...
    async def _get_post(self, idx: int) -> Union[Post, None, False]:
        ...
        try:
            ...
            # ZCode: 非中文内容翻译为中文
            if post.title or post.html:
                try:
                    new_title, new_html = await translate_post(post.title, post.html)
                    if new_title != post.title or new_html != post.html:
                        post.title = new_title
                        post.html = new_html
                        # 重建 formatter 以反映翻译后的内容
                        post.post_formatter = PostFormatter(
                            html=post.html,
                            title=post.title,
                            ...
                            pub_time=post.pub_time,     # ZCode
                            push_time=post.push_time,   # ZCode
                        )
                except Exception as e:
            ...
            try:
                ...
                try:
                    if isinstance(post, Post):
                        await record_push(
                            feed_title=post.feed_title or sub.feed.title if hasattr(sub, 'feed') else '',
