                ...
                for entry in updated_entries:
                    ...
                    if pub:
                        try:
                            ...
                            filtered.append(entry)  # 无法判断时间的保留
                    else:
                        filtered.append(entry)  # 无时间戳的保留
                if len(filtered) < len(updated_entries):
                    logger.debug(f'24h filter: {len(updated_entries) - len(filtered)} entries skipped (older than 24h)')
                updated_entries = filtered
            # ---- end ZCode ----
