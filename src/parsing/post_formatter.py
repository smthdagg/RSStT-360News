...
from zoneinfo import ZoneInfo  # ZCode: 北京时间
from html import escape as html_escape

# ZCode: 北京时间时区
BJT = ZoneInfo('Asia/Shanghai')

def _fmt_bj(dt: Optional[datetime]) -> str:
    """将 UTC 时间转为北京时间字符串 (MM-DD HH:MM)"""
    if dt is None:
        return ''
...
def _time_diff_str(pub: datetime, push: datetime) -> str:
    """计算时间差并格式化为中文"""
    delta = push - pub
    ...
    if total_mins < 0:
        return '0分钟'
    if total_mins < 60:
        return f'{total_mins}分钟'
    elif total_mins < 1440:
        return f'{total_mins // 60}小时{total_mins % 60}分钟'
    else:
        return f'{total_mins // 1440}天{total_mins % 1440 // 60}小时'
...
class PostFormatter:
    def __init__(self,
                 ...
                 pub_time: Optional[datetime] = None,   # ZCode
                 push_time: Optional[datetime] = None):  # ZCode
        """
        ...
        :param pub_time: original publish time (UTC)
        :param push_time: push/delivery time (UTC)
        """
        self.html = html
        self.title = title
        ...
        self.pub_time = pub_time      # ZCode
        self.push_time = push_time    # ZCode
        ...
        if self.enclosures:
            ...
            for enclosure in self.enclosures:
                ...
                if not utils.isAbsoluteHttpLink(enclosure.url):
                    ...
                    # ZCode: 本地截图 (127.0.0.1) 直接下载后上传，而非给 Telegram 外链
                    if '127.0.0.1' in enclosure.url or 'localhost' in enclosure.url:
                        medium = self._make_local_image(enclosure.url)
                    else:
                        medium = Image(enclosure.url)
                else:
                    medium = File(enclosure.url)
    ...
    def _make_local_image(self, url: str):
        """下载本地截图 (127.0.0.1) 并返回 UploadedImage"""
        try:
            import urllib.request
            resp = urllib.request.urlopen(url, timeout=10)
            if resp.status == 200:
                data = resp.read()
                from .medium import UploadedImage
                logger.debug(f'Local screenshot downloaded: {url} ({len(data)} bytes)')
                return UploadedImage(data, 'screenshot.png')
        except Exception as e:
            logger.warning(f'Failed to download local screenshot {url}: {e}')
            from .medium import Image
        return Image(url)
