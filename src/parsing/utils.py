...
async def parse_entry(entry, feed_link: Optional[str] = None):
    ...
    if isinstance(content, list) and len(content) > 0:  # Atom
        # ZCode: 优先取最完整的内容（接受所有类型，不光 html）
        # RSSHub X/Twitter 输出 content 为 text/plain，不应跳过
        best = None
        best_len = 0
        for _content in content:
            val = _content.get('value', '')
            if len(val) > best_len:
                best = val
                best_len = len(val)
        content = best or content[0].get('value', '')
    ...
    # ZCode: 如果内容过短但标题较长（如 X/Twitter 截断情况），用标题替代
    if len(content) < 50 and entry.get('title') and len(entry['title']) > len(content):
        content = entry['title']

    EntryParsed.content = await html_validator(content)
    EntryParsed.link = entry.get('link') or entry.get('guid')

    # ZCode: 如果内容太单薄（< 800 字符或大部分是链接的元信息），尝试从原文抓取正文
    link = EntryParsed.link
    needs_fetch = False
    if link and link.startswith('http'):
        content_len = len(EntryParsed.content)
        if content_len < 800:
            needs_fetch = True
        elif content_len < 3000:
            # 检测是否大部分是链接/元信息（没有实际段落）
            text_only = EntryParsed.content.replace('</p>', '\n').replace('<br/>', '\n').replace('<br />', '\n')
            text_only = ''.join(EntryParsed.content.split('>')[-1].split('<')[0] for _ in range(1))
            import re
            stripped = re.sub(r'<[^>]+>', '', EntryParsed.content)
            # 计算纯文本中链接占比
            urls = re.findall(r'https?://\S+', stripped)
            url_chars = sum(len(u) for u in urls)
            text_chars = len(stripped)
            if text_chars > 0 and url_chars / text_chars > 0.5:
                needs_fetch = True
    if needs_fetch:
        try:
            import trafilatura
            def _fetch_extract(url):
                ...
                if downloaded:
                    return trafilatura.extract(downloaded, output_format='html',
                                                include_links=True, include_images=False)
                return None
            fetched = await asyncio.get_event_loop().run_in_executor(
                None, _fetch_extract, link
            )
            if fetched and len(fetched) > len(EntryParsed.content):
                EntryParsed.content = fetched
        except Exception:
            pass  # 抓取失败，使用原标题+链接
